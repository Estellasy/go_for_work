...
1. 通用默认格式 %v:
   1. %v和%+v的区别:%v和%+v都是格式化输出的动词,%+v用于格式化输出类型,在应用结构体时,%+v会额外输出结构体的字段名,而%v不会.
   2. %#前缀,输出结构体时,%#v会输出结构体的源码格式,%#+v会输出结构体的源码格式,并额外输出结构体的字段名.
   3. %#v和%#+v的区别:%#v和%#+v都是格式化输出动词,%#v用于格式化输出类型,在应用结构体时,%#v会输出结构体的源码格式,而%#+v会额外输出结构体的字段名.
2. %T:打印值的类型
3. %d:十进制整数
4. %b:二进制整数
5. %x:十六进制整数
...
### 带缓冲channel和无缓冲channel的异同点

容量：

带缓冲的通道有一个容量限制，即可以存储一定数量的元素。
无缓冲的通道没有容量限制，每个发送操作必须等待对应的接收操作完成，反之亦然。
阻塞：

对于带缓冲的通道，发送操作只有在通道满时才会阻塞，接收操作只有在通道空时才会阻塞。
对于无缓冲的通道，发送操作和接收操作都会导致阻塞，直到另一个Go协程对应的操作完成为止。这使得通道成为了同步的一种工具。
异步操作：

带缓冲的通道可以支持异步操作，即发送或接收操作可以立即完成而无需等待对方就绪。
无缓冲的通道是同步的，发送和接收操作都需要等待对方准备好才能完成，因此无法进行异步操作。
性能：

通常情况下，带缓冲的通道具有更好的性能，因为发送和接收操作可以在没有阻塞的情况下完成，而无需等待对方准备好。
无缓冲的通道由于需要等待对方就绪才能完成操作，可能会引入更多的等待时间，因此性能上可能稍逊于带缓冲的通道。
使用场景：

带缓冲的通道适用于生产者-消费者模型中，当发送和接收操作的速度不一致时，可以缓解两者之间的压力。
无缓冲的通道适用于强同步的场景，如控制并发的数量、协调多个Go协程的执行顺序等。
...
使用上下文,如使用context包创建具有超时/取消等功能的上下文,更好地管理协程
